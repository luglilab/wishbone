...
class SCData(object):
    ...
    def from_csv(cls, counts_csv_file, data_type, cell_axis=0, normalize=True):
        ...
        # Read in csv file
        df = pd.read_csv(
            counts_csv_file, sep=None, header=0, index_col=0, engine="python"
        )
    ...
    def run_tsne(
        self, n_components: int = 15, perplexity: int = 30, rand_seed: int = -1
    ) -> None:
        """\
        Run tSNE on the data. tSNE is run on the principal component projections
        for single cell RNA-seq data and on the expression matrix for mass cytometry
        data

        :param n_components:
            Number of components to use for running tSNE for single cell RNA-seq
            data. Ignored for mass cytometry
        :param perplexity:
        :param rand_seed:
        ...
        if self.data_type == "sc-seq":
            if self.pca is None:
                raise RuntimeError(
                    "Please run PCA using run_pca before running tSNE for single cell "
                    "RNA-seq"
                )
            data -= np.min(np.ravel(data))
            data /= np.max(np.ravel(data))
            data = pd.DataFrame(
                np.dot(data, self.pca["loadings"].iloc[:, 0:n_components]),
    ...
    def run_diffusion_map(
        ...
        :param knn:
            Number of neighbors for graph construction to determine distances between
            cells
        :param epsilon:
            Gaussian standard deviation for converting distances to affinities
        :param n_diffusion_components:
            Number of diffusion components to Generalte
        :param n_pca_components:
            Number of components to use for running tSNE for single cell RNA-seq
            data. Ignored for mass cytometry
        :param markers:
        ...
        if self.data_type == "sc-seq":
            if self.pca is None:
                raise RuntimeError(
                    "Please run PCA using run_pca before running diffusion maps for "
                    "single cell RNA-seq"
                )
        ...
        # Nearest neighbors
        N = data.shape[0]
        nbrs = sklearn.neighbors.NearestNeighbors(n_neighbors=knn).fit(data)
        ...
        for i in range(N):
            ...
            location += knn
        W = scipy.sparse.csr_matrix((dists, (rows, cols)), shape=[N, N])

        # Symmetrize W
        W = W + W.T

        # Convert to affinity (with selfloops)
        rows, cols, dists = scipy.sparse.find(W)
        rows = np.append(rows, range(N))
        cols = np.append(cols, range(N))
        dists = np.append(dists / (epsilon ** 2), np.zeros(N))
        W = scipy.sparse.csr_matrix((np.exp(-dists), (rows, cols)), shape=[N, N])
        ...
        D = scipy.sparse.csr_matrix((np.sqrt(D), (range(N), range(N))), shape=[N, N])
        P = D
        T = D.dot(W).dot(D)
        T = (T + T.T) / 2

        # Eigen value decomposition
        D, V = scipy.sparse.linalg.eigs(
            T, n_diffusion_components, tol=1e-4, maxiter=1000
        )
        D = np.real(D)
        V = np.real(V)
        inds = np.argsort(D)[::-1]
...
class Wishbone:
    ...
    def run_wishbone(
        ...
        :param start_cell:
            Desired start cell. This has to be a cell in `~Wishbone.scdata.index`
        :param branch:
        ...
        # Run the algorithm
        res = c_wishbone(
            self.scdata.diffusion_eigenvectors.loc[:, components_list].values,
            s=s,
            k=k,
            l=k,
        ...
        if branch:
            self.branch = pd.Series(
                [np.int32(i) for i in branches], index=self.scdata.data.index
            )
    ...
    def plot_marker_trajectory2(
        ...
        # Marker colors
        #colors = plt.cm.get_cmap('tab10', len(markers)).colors  # Using tab10 colormap for distinct colors
        colors = qualitative_colors(len(markers))
        scaling_factor = 0.5
        linewidth = 3

        # Set up plot
        fig, ax = plt.subplots(figsize = [14, 4]) if fig is None else (fig, ax)
        ...
        for marker, color in zip(markers, colors):
            ...
            if not plot_branch:
                ...
                # Plot
                ax.plot(xaxis, vals, label = marker, color = color, linewidth = linewidth)
                ...
                if show_variance:
                    ...
                    wstds = (
                            np.inner1d(np.asarray(temp).T, np.asarray(weights).T)
                            / weights.sum()
                    )
                ...
                for br in branches:
                    ...
                    plot_vals = ((rep_mark * weights) / np.sum(weights)).sum()
                    branch_vals.append(plot_vals[(bp_bin - 1):].to_list())

                # Min and max
                trunk_vals = trunk_vals.to_list()
                temp = trunk_vals + branch_vals[1] + branch_vals[0]
                min_val = min(temp)
                max_val = max(temp)
                # Plot the trunk
                plot_vals = ((rep_mark * weights) / np.sum(weights)).sum()
                ...
                if show_variance:
                    ...
                    # weighted_variance = np.dot(temp.T, weights) / weights.sum()
                    # wstds = np.sqrt(weighted_variance)
                    # Normalize and scale the marks along axis 1
                    temp = ((norm_rep_mark - plot_vals - np.min(y)) / np.max(y)) ** 2

                    # Ensure weights are aligned properly
                    weights = weights.values  # Convert to numpy array if it is a pandas Series

                    # Compute weighted variance along axis 1
                    weighted_variance = (temp * weights).sum(axis = 1) / weights.sum()

                    # Compute weighted standard deviation
                    wstds = np.sqrt(weighted_variance)

                    # Convert back to pandas Series if needed
                    wstds = pd.Series(wstds)
                ...
                if (
                    ...
                    if show_variance:
                        ...
                        # Ensure weights are also flattened if needed
                        weights = weights.to_numpy().flatten()  # Or weights.values.flatten() if using deprecated .values

                        # Calculate weighted variance along axis 1
                        weighted_variance = np.dot(temp, weights) / weights.sum()
                        ...
                        ax.fill_between(
                            xaxis[0:bp_bin],
                            plot_vals[0:bp_bin] - scaling_factor * wstds,
                            plot_vals[0:bp_bin] + scaling_factor * wstds,
                            alpha = 0.1,
                            color = color,
                        )
        ...
        # Clean xlim
        ax.legend(loc = 2, bbox_to_anchor = (1, 1), prop = {"size": 16})
        ...
        ax.set_ylim([-0.2, 1.1])
        ax.set_xlabel("Wishbone trajectory")
    ...
    def plot_marker_heatmap(
        ...
        if self.branch is not None:
            ...
            for i, br in enumerate(branches):
                ...
                # Concatenate the DataFrames without resetting the index
                mat = pd.concat([marker_trends["Trunk"], marker_trends[br].iloc[2:]])
                # Reset the index manually
                ...
                tick_positions = [np.where(plot_mat["x"] >= i)[0][0] for i in ticks]
                plt.xticks(tick_positions, [round(i, 1) for i in ticks])
                # Labels
                plt.xlabel("Wishbone trajectory")
                plt.title(br)
        else:
    ...
    def plot_derivatives(
        ...
        if self.branch is not None:
            ...
            for i, br in enumerate(branches):
                ...
                # Define ticks and their positions
                ticks = np.arange(trajectory_range[0], trajectory_range[1] + 0.1, 0.1)
                tick_positions = [np.where(mat["x"] >= i)[0][0] for i in ticks]

                # Setting the x-ticks with rounded float labels
                plt.xticks(tick_positions, [round(i, 1) for i in ticks])
